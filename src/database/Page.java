	...
	public static void insertLeafCell(RandomAccessFile file, int page, int offset, short plsize, int key, byte[] stc, String[] vals, String table)
	{
		try
		{
			...
			if(!table.equals("davisbase_columns") && !table.equals("davisbase_tables"))
			{
				...
				RandomAccessFile IndexFile = new RandomAccessFile("data\\"+DavisBase.currentDatabase+"\\"+table+"\\"+colName[0]+".ndx", "rw");
				...
				{
					IndexFile = new RandomAccessFile("data\\"+DavisBase.currentDatabase+"\\"+table+"\\"+colName[i]+".ndx", "rw");
					IndexFile.seek(IndexFile.length());
					switch(stc[i-1])
					{
						case 0x00:
